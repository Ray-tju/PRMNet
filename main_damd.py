...
def main(args):
    ...
    checkpoint_fp = '/media/pc/D/lilei/3DDFA-master/model/DAMDNet.pth.tar'
    arch = 'DAMDNet_v1'
    ...
    for img_fp in args.files:
        ...
        for rect in rects:
            ...
            if args.dump_ply:
                dump_to_ply(vertices, tri, '{}_{}_PIR.ply'.format(img_fp.replace(suffix, ''), ind))
            if args.dump_vertex:
            ...
            if args.dump_obj:
                wfp = '{}_{}_damd.obj'.format(img_fp.replace(suffix, ''), ind)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=img_fp.replace(suffix, 'DAMD.jpg'), show_flg=args.show_flg)
