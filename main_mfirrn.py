...
def main(args):
    ...
    checkpoint_fp = '/media/pc/D/lilei/3DDFA-master/model/mfirrn.tar'
    checkpoint = torch.load(checkpoint_fp, map_location=lambda storage, loc: storage)['state_dict']
    model = getattr(mfirrn, 'resnet34')(num_classes=62)# 62 = 12(pose) + 40(shape) +10(expression)
    ...
    for img_fp in args.files:
        ...
        for rect in rects:
            ...
            with torch.no_grad():
                ...
                output, output_medium, output_fine, param = model(input, input1, input2)
            ...
            if args.dump_ply:
                dump_to_ply(vertices, tri, '{}_{}_ssrnet.ply'.format(img_fp.replace(suffix, ''), ind))
            if args.dump_vertex:
            ...
            if args.dump_obj:
                wfp = '{}_{}_mfirrn.obj'.format(img_fp.replace(suffix, ''), ind)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=img_fp.replace(suffix, '_mfirrn.jpg'), show_flg=args.show_flg)
