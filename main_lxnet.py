...
def main(args):
    ...
    checkpoint_fp = '/media/lab280/D/lilei/3DDFA_3.52/training/snapshot_lx/phase1_wpdc_lx_checkpoint_epoch_36.pth.tar'
    checkpoint = torch.load(checkpoint_fp, map_location=lambda storage, loc: storage)['state_dict']
    model = repvgg()# 62 = 12(pose) + 40(shape) +10(expression)
    ...
    for img_fp in args.files:
        ...
        for rect in rects:
            ...
            if args.dump_obj:
                wfp = '{}_{}_lxnet.obj'.format(img_fp.replace(suffix, ''), ind)
        ...
        if args.dump_pncc:
            ...
            print('Dump to {}'.format(wfp))
        if args.dump_res:
            draw_landmarks(img_ori, pts_res, wfp=img_fp.replace(suffix, '_lxnet.jpg'), show_flg=args.show_flg)
