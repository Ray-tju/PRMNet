...
def train(train_loader, model, criterion, criterion1, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        input1 = jigsaw_generator(input, 3)
        # A.Compose([
        ...
        #     A.Rotate()
        # ])
        input_cat = torch.cat((input, input1), dim=0)
        ...
        losses1.update(loss1.item(), 1)
        losses2.update(loss2.item(), 1)
        # losses3.update(loss3.item(), input.size(0))
        # losses3.update(loss_concate.item(), input.size(0))
        ...
        if i % args.print_freq == 0:
            logging.info(f'Epoch: [{epoch}][{i}/{len(train_loader)}]\t'
                         ...
                         f'Loss_wpdc {losses1.val:.4f} ({losses1.avg:.4f})'
                         f'loss_wing{losses2.val:.4f} ({losses2.avg:.4f})'
                         # f'Loss_vdc {losses3.val:.4f} ({losses3.avg:.4f})'
...
def main():
    ...
    if args.loss.lower() == 'wpdc':
        ...
        # criterion2 = VDCLoss(opt_style=args.opt_style).cuda()
    ...
    if args.test_initial:
        ...
        validate(val_loader, model, criterion, criterion1, criterion2, args.start_epoch)
