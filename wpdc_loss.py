...
class WPDCLoss(nn.Module):
    ...
    def __init__(self, opt_style='resample', resample_num=132):
        ...
        self.u_base = _to_tensor(u_base)
        self.w_exp_base = _to_tensor(w_exp_base)
        self.w_shp_base = _to_tensor(w_shp_base)
        self.u = _to_tensor(u)
    ...
    def forward(self, input, target, weights_scale=10):
        if self.opt_style == 'resample':
            ...
            gt_vertexG = pg @ (self.u + self.w_shp @ alpha_shpg + self.w_exp @ alpha_expg) \
                .view(N, -1, 3).permute(0, 2, 1) + offsetg
            vertexG = p @ (self.u + self.w_shp @ alpha_shp + self.w_exp @ alpha_exp) \
                .view(N, -1, 3).permute(0, 2, 1) + offset

            gt_vertexL_P = pg @ (self.u_base + self.w_shp_base @ alpha_shpg + self.w_exp_base @ alpha_expg) \
                .view(N, -1, 3).permute(0, 2, 1) + offsetg
            vertexL_P = p @ (self.u_base + self.w_shp_base @ alpha_shp + self.w_exp_base @ alpha_exp) \
                .view(N, -1, 3).permute(0, 2, 1) + offset

            diffG = (gt_vertexG - vertexG) ** 2
            diffL_P = (gt_vertexL_P - vertexL_P) ** 2

            lossG = torch.mean(diffG) / 1000
            lossL_P = torch.mean(diffL_P) / 10000
            return (loss.mean() + lossG + lossL_P) / 3
            # return (loss.mean() + lossG + lossL_P) / 3
        else:
